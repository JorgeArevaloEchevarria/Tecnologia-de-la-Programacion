...
public class Main {
	 ...
	 * Default mode to use.
	 */
	final private static AppMode DEFAULT_MODE = AppMode.NORMAL;
	...
	private static AppMode applicationMode = DEFAULT_MODE;
	...
	private static void parseObstaclesOptions(CommandLine line) throws ParseException {
		...
		if(obsVal != null){ //Si no ponemos nada no hay obstaculos
			try {
				String numObstaculos = obsVal;
				obstacles = Integer.parseInt(numObstaculos);
				
			}catch (NumberFormatException e) {
				throw new ParseException("No es un numero valido: " + obstacles);
			}
		}
	}
	
	private static Option constructObstaclesOption(){
		String optionInfo = "The number of obstacles in the game ";
		optionInfo += "By default, there aren't any obstacles."; 
	...
	private static Option constructAppModeOption(){
		String optionInfo = "The mode of the application";
		optionInfo += "By default is the normal mode";
		Option opt = new Option("am","app-mode",true,optionInfo);
		return opt;
	}
	/**
	 * parsea la opcion recibida
	 * @param line
	 * @throws ParseException
	 */
	private static void parseAppModeOption(CommandLine line) throws ParseException{
		String mode = line.getOptionValue("am");
		if(mode != null)
			switch(mode){
			
			case "server":
				applicationMode = AppMode.SERVER;
				break;
			
			case "client":
				applicationMode = AppMode.CLIENT;
				break;
			
			default:
				applicationMode = AppMode.NORMAL;
	...
	private static Option constructServerPortOption(){
		String optionInfo = "The server port that the application will use";
		
		optionInfo += ".The default port is" + DEFAULT_PORT;
		
		Option opt = new Option("sp","server-port",true,optionInfo);
		return opt;
	}
	
	/**
	 * parsea la opcion de cliente o servidor
	 * @param line
	 */
	private static void parseServerPortOption(CommandLine line){
		String sp = line.getOptionValue("sp");

		if(sp != null && (applicationMode == AppMode.CLIENT || applicationMode == AppMode.SERVER))
	...
	private static Option constructServerHostOption(){
		String optionInfo = "The server host that the application will use";
		optionInfo += ".The default host is" + DEFAULT_HOST;
		
		Option opt = new Option("sh","server-host",true,optionInfo);
		return opt;
	}
	/**
	 * parsea el host
	...
	private static void parseServerHostOption(CommandLine line){
		String sh = line.getOptionValue("sh");
		
		if(sh != null && applicationMode == AppMode.CLIENT)
			serverHost = sh;
	...
	public static void startGame() {
		...
		case WINDOW:
			...
			System.out.println("Ha entrado a window");
			break;
