...
public class TttState extends GameState<TttState, TttAction> {
    ...
    public static boolean isWinner(int[][] board, int playerNumber) {
        ...
        for (int i=0; !won && i<board.length; i++) {
            if (isWinner(board, playerNumber, 0, i, 1, 0)) won = true; // recorre hacia abajo
            if (isWinner(board, playerNumber, i, 0, 0, 1)) won = true; // recorre hacia la derecha
        }
        return won ||
                isWinner(board, playerNumber, 0, 0, 1, 1) || //recorre la diagonal principal de izquierda a derecha
                isWinner(board, playerNumber, 2, 0, -1, 1); //recorre la diagonal secundaria de izquierda a deracha
    }    
